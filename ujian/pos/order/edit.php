...
<body>
	...
	<div id="main" class="clearfix">
		...
		include '../koneksi.php';
		$id 	= $_GET['id'];
		$snl	= "SELECT * FROM pesanan WHERE id = '$id'";
		$masdar	= mysqli_query($koneksi, $snl);
		$mor	= mysqli_fetch_assoc($masdar);

		$ck		= $mor['item_id'];
		$mcl 	= "SELECT * FROM item WHERE id = '$ck'";
		$cs 	= mysqli_query($koneksi, $mcl);
		$bv		= mysqli_fetch_assoc($cs);
			...
			<table>
				...
				<tr>
					...
					<td><select name="item">
						...
						<option value="<?php echo $mor['item_id'] ?>">--<?php echo $bv['name']?>--</option>
						<?php
							include '../koneksi.php';
